...
def init_config(session):    
    ...
    session.add(Config(classes_per_day=5, days_per_week=5, hour_names=json.dumps(DEFAULT_HOUR_NAMES), day_indices=json.dumps(DEFAULT_DAY_INDICES)))  
...
def init_dummy_data(session):    
    ...
    math1 = Subject(id="MAT1", name="Matemáticas", weekly_hours=6, max_hours_per_day=2, course=course_1)
    lang1 = Subject(id="LEN1", name="Lengua", weekly_hours=6, max_hours_per_day=2, course=course_1)
    val1 = Subject(id="VAL1", name="Valores", weekly_hours=1, max_hours_per_day=1, course=course_1)
    rel1 = Subject(id="REL1", name="Religión", weekly_hours=1, max_hours_per_day=1, course=course_1)
    sci1 = Subject(id="CIE1", name="Ciencias", weekly_hours=2, max_hours_per_day=1, course=course_1)
    eng1 = Subject(id="ING1", name="Inglés", weekly_hours=4, max_hours_per_day=1, course=course_1)
    mus1 = Subject(id="MUS1", name="Música", weekly_hours=1, max_hours_per_day=1, course=course_1)
    art1 = Subject(id="ART1", name="Educación Plástica", weekly_hours=1, max_hours_per_day=1, course=course_1)
    soc1 = Subject(id="SOC1", name="Sociales", weekly_hours=2, max_hours_per_day=1, course=course_1)
    ef1 = Subject(id="EFI1", name="Educación Física", weekly_hours=2, max_hours_per_day=1, course=course_1)
    subjects1 = [
        math1,
        lang1,
        val1,
    ...
    for subject in subjects1:
        print(f"Subject: {subject.name} -(ID: {subject.id}) - {subject.weekly_hours} weekly hours - max:{subject.max_hours_per_day} hours/day")
