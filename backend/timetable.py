...
from .models import TimeSlotAssignment, Timeslot, Config
...
def get_timetables_from_db(session):
    ...
    for assignment in assignments:
        ...
        course_line = f"{timeslot.course_id}{chr(ord('A') + timeslot.line)}"   # e.g., "1ºA"
        hour = timeslot.hour       # e.g., 1, 2, ...
        # weekday stored as an index (0 = first weekday)
        weekday = timeslot.day
        subject_name = assignment.subject.name
        teacher_name = assignment.teacher.name
        timetable[course_line][(hour, weekday)].append(f"{subject_name} ({teacher_name})")
    return timetable
...
def print_markdown_timetable_from_assignments(session) -> str:
    ...
    for course_line in sorted(timetable.keys()):
        ...
        day_indices = cfg_dict.get('day_indices') if cfg_dict else list(range(cfg.days_per_week if cfg else 5))
        ...
        for hour in sorted_hours:
            ...
            for day_index in range(len(weekdays)):
                ...
                if assignments:
                    ...
                    cell = ""
                row.append(cell)
            hour_label = cfg_hour_names[hour] if hour < len(cfg_hour_names) else t('hours.label').format(n=hour+1)
...
def get_teacher_timetables_from_db(session):
    ...
    for assignment in assignments:
        ...
        teacher_timetable[teacher_name][(hour, weekday)].append(f"{course_line}: {subject_name}")
